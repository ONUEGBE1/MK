...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print('\n\n %s[%s#%s] crack selesai...\n'%(N,K,N))
        print(' [%s+%s] TOTAL OK : %s%s%s'%(O,K,H,str(len(ok)),N))
        print(' [%s+%s] TOTAL CP : %s%s%s'%(O,K,K,str(len(cp)),N))
...
def moch_yayan():
    ...
    print(' [%s01%s]. CRACK FROM GROUP'%(O,K));time.sleep(0.03)
    print(' [%s02%s]. CRACK FROM PUBLIC'%(O,K));time.sleep(0.03)
    print(' [%s03%s]. CRACK FROM FOLLOWERS'%(O,K));time.sleep(0.03)
    print(' [%s04%s]. CRACK FROM LIKES'%(O,K));time.sleep(0.03)
    print(' [%s05%s]. CRACK FROM RAMDOM IDs'%(O,K));time.sleep(0.03)
    print(' [%s06%s]. CRACK FROM POST'%(O,K));time.sleep(0.03)
    print(' [%s07%s]. HACK NEW INSTAGRAM [%snew%s]'%(O,N,H,N));time.sleep(0.03)
    print(' [%s08%s]. CHECKPOINT REVIEW'%(O,K));time.sleep(0.03)
    print(' [%s09%s]. SETTINGS USER AGENTS'%(O,K));time.sleep(0.03)
    print(' [%s10%s]. CHECK CRACK RESULTS'%(O,K));time.sleep(0.03)
    print(' [%s11%s]. Upgrade ke%s premium%s'%(O,K,O,K));time.sleep(0.03)
    print(' [%s00%s]. logout (%shapus cookie%s)'%(O,K,O,K));time.sleep(0.03)
    pepek = input('\n [%s*%s] menu : '%(H,N))
    if pepek == '':
